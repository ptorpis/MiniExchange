...
class MiniExchangeClient(App):
    ...
    def on_mount(self) -> None:
        ...
        self.client = miniexchange_client.MiniExchangeClient(api_key)
    ...
    def handle_message(self, msg: dict) -> None:
        ...
        if msg_type == "HELLO_ACK":
            ...
            log.write(
                f"[bold cyan]< HELLO_ACK[/bold cyan] |"
            ...
            if price == 0:
                log.write(
                    f"[bold yellow]< ORDER_ACK[/bold yellow] |"
            ...
            log.write(
                f"[bold magenta]< TRADE[/bold magenta] |"
            ...
            order_id = msg.get("server_order_id", "?")
            log.write(
                f"[bold red]< CANCEL_ACK[/bold red] "
                f"| Status: {status} | Order ID: {order_id}"
            )
            ...
            log.write(
                f"[bold blue]< MODIFY_ACK[/bold blue] "
            ...
            if cmd == "hello" or cmd == "login":
                ...
                if is_limit:
                    ...
                    if len(parts) >= 5:
                        ...
                        self.client.send_order(qty, price, is_buy, is_limit)  # type: ignore
                        log.write(
                            f"[dim]Sent ORDER: {qty} ({side}, {order_type})[/dim]"
                        )
                else:
                ...
                if not orders:
                    ...
                    for order_id, order in orders.items():
                        ...
                        log.write(
                            f"{order_id:<8} {order['qty']:<6d} {order['price']:<8d} {created:<12.3f}"
                        )
                ...
                log.write(
                    "  order <buy|sell> <qty> <limit|market> [price] - Send order"
                )
